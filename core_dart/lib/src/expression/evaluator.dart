...
class ExpressionEvaluator {
  ...
  //const factory ExpressionEvaluator.async({List<MemberAccessor> memberAccessors}) = AsyncExpressionEvaluator;
  ...
  dynamic evalLiteral(Literal literal, Map<String, dynamic> context) {
    ...
    if (value is Map) {
      return value.map((key, value) => MapEntry(eval(key, context), eval(value, context)));
    }
    return value;
  }
  ...
  dynamic getMember(dynamic obj, String member) {
    ...
    throw ExpressionEvaluatorException.memberAccessNotSupported(obj.runtimeType, member);
  }
}
...
class ExpressionEvaluatorException implements Exception {
  ...
  ExpressionEvaluatorException.memberAccessNotSupported(Type type, String member)
      : this(
...
abstract class MemberAccessor<T> {
  static const MemberAccessor<Map> mapAccessor = MemberAccessor<Map>.fallback(_getMapItem);

  static dynamic _getMapItem(Map map, String key) => map[key];

  const factory MemberAccessor(Map<String, SingleMemberAccessor<T>> accessors) = _MemberAccessor;

  const factory MemberAccessor.fallback(AnyMemberAccessor<T> accessor) = _MemberAccessorFallback;
