...
class ExpressionParser {
  ExpressionParser() {
    expression.set(binaryExpression
        ...
        .map((l) => l[1] == null ? l[0] : ConditionalExpression(l[0], l[1][0], l[1][1])));
  ...
  Parser<Identifier> get identifier =>
      (digit().not() & (word() | char(r"$")).plus()).flatten().map((v) => Identifier(v));
  ...
  Parser<String> get escapedChar => (char(r"\") & anyOf("nrtbfv\"'\\")).pick(1).cast();
  ...
  Parser<Literal> get sqStringLiteral => (char("'") & (anyOf(r"'\").neg() | escapedChar).star().flatten() & char("'"))
      .pick(1)
      .map((v) => Literal(unescape(v), "'$v'"));

  Parser<Literal> get dqStringLiteral => (char('"') & (anyOf(r'"\').neg() | escapedChar).star().flatten() & char('"'))
      .pick(1)
      .map((v) => Literal(unescape(v), '"$v"'));
  ...
  Parser<Literal> get boolLiteral => (string("true") | string("false")).map((v) => Literal(v == "true", v));

  // Parses the null literal
  Parser<Literal> get nullLiteral => string("null").map((v) => Literal(null, v));

  // Parses the this literal
  Parser<ThisExpression> get thisExpression => string("this").map((v) => ThisExpression());

  // Responsible for parsing Array literals `[1, 2, 3]`
  // This function assumes that it needs to gobble the opening bracket
  // and then tries to gobble the expressions as arguments.
  Parser<Literal> get arrayLiteral =>
      (char("[").trim() & arguments & char("]").trim()).pick(1).map((l) => Literal(l, "$l"));

  Parser<Literal> get mapLiteral =>
      (char("{").trim() & mapArguments & char("}").trim()).pick(1).map((l) => Literal(l, "$l"));

  Parser<Literal> get literal =>
      (numericLiteral | stringLiteral | boolLiteral | nullLiteral | arrayLiteral | mapLiteral).cast();
  ...
  Parser<String> get binaryOperation =>
      binaryOperations.keys.map<Parser<String>>((v) => string(v)).reduce((a, b) => (a | b).cast<String>()).trim();

  Parser<Expression> get binaryExpression => token.plusSeparated(binaryOperation).map((sl) {
        ...
        for (var i = 1; i < l.length; i += 2) {
          ...
          while ((stack.length > 2) && (prec <= BinaryExpression.precedenceForOperator(stack[stack.length - 2]))) {
  ...
  Parser<List<Expression>> get arguments =>
      expression.plusSeparated(char(",").trim()).map((sl) => sl.elements).castList<Expression>().optionalWith([]);

  Parser<Map<Expression, Expression>> get mapArguments => (expression & char(":").trim() & expression)
  ...
  Parser<Expression> get variable =>
      groupOrIdentifier.seq((memberArgument.cast() | indexArgument | callArgument).star()).map((l) {
  ...
  Parser<Expression> get group => (char("(") & expression.trim() & char(")")).pick(1).cast();

  Parser<Expression> get groupOrIdentifier => (group | thisExpression | identifier.map((v) => Variable(v))).cast();

  Parser<Identifier> get memberArgument => (char(".") & identifier).pick(1).cast();

  Parser<Expression> get indexArgument => (char("[") & expression.trim() & char("]")).pick(1).cast();

  Parser<List<Expression>> get callArgument => (char("(") & arguments.trim() & char(")")).pick(1).cast();

  // Ternary expression: test ? consequent : alternate
  Parser<List<Expression>> get conditionArguments =>
      (char("?").trim() & expression.trim() & char(":").trim()).pick(1).seq(expression).castList();
