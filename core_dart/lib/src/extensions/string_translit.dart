// https://github.com/alexeynobody/translit-dart/blob/master/lib/translit.dart
...
extension StringTranslit on String {
  /// Method for converting from translit for the [source] value
  String? _untranslit({String? source}) {
    ...
    for (final element in sourceSymbols) {
      unTranslit.add(deTransliteratedSymbol.containsKey(element) ? deTransliteratedSymbol[element] : element);
    }
  ...
  String? _translit({String? source}) {
    if (source == null || source.isEmpty) return source;
    ...
    for (final element in sourceSymbols) {
      translit.add(_transliteratedSymbol.containsKey(element) ? _transliteratedSymbol[element] : element);
    }

    return translit.join();
  }

  String? translit() => _translit(source: this);

  String? untranslit() => _untranslit(source: this);
}
...
extension RemoveDiacritics on String {
  String removeDiacritics() => String.fromCharCodes(_replaceCodeUnits(codeUnits));
  int compareToIgnoringDiacritics(String other) => removeDiacritics().compareTo(other.removeDiacritics());
}
