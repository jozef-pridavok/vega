...
class Indentation {
  ...
  String trimMargin([String marginPrefix = "|"]) {
    ...
    for (final line in lines) {
      ...
      if ((i == 0 || i == lines.length - 1) && leftTrimmedLine.trimRight().isEmpty) {
  ...
  Iterable<_Line> _processLines() sync* {
    ...
    for (final line in LineSplitter.split(_input)) {
      ...
      final indentationLevel = indentationMatch != null && indentationMatch.isNotEmpty ? indentationMatch.length : 0;
  ...
  int _findCommonIndentationLevel(Iterable<_Line> lines) {
    ...
    for (final line in lines) {
      ...
      if (commonIndentationLevel == null || line.indentationLevel < commonIndentationLevel) {
