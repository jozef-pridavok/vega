...
class Localization {
  ...
  String _resolve(String key, {bool logging = true, bool fallback = true}) {
    ...
    if (resource == null) {
      if (logging) {
        LocalizedWidget.logger.warning("Localization key [$key] not found");
      }
      if (_fallbackTranslations == null || !fallback) {
        ...
        if (resource == null) {
          if (logging) {
            LocalizedWidget.logger.warning("Fallback localization key [$key] not found");
          }
          return key;
        }
      }
