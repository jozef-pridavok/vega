...
class WeekStackedBarChart extends ConsumerWidget with ChartDefaults {
  final String title;
  ...
  const WeekStackedBarChart(
      {required this.title, required this.subtitle, required this.values, required this.colors, required this.labels});
  ...
  List<charts.Series<_WeekChartData, String>> _createData(BuildContext context, WidgetRef ref) {
    ...
    for (int set = 0; set < values.length; set++) {
      week.forEachIndexed((index, day) {
        final negative = set == 3 ? -1 : 1;
        data[set].add(_WeekChartData(
          _getShortName(format, day),
          (values[set])?[index] != null ? (values[set])![index] * negative : null,
        ));
      });
    }

    //print("data: ${data.map((e) => e.map((e) => e.count).toList()).toList()}");
