...
class FinishedProgramsWidget extends ConsumerStatefulWidget {
  const FinishedProgramsWidget({super.key});

  @override
  createState() => _WidgetState();
}

class _WidgetState extends ConsumerState<FinishedProgramsWidget> {
  @override
  void initState() {
    super.initState();
    Future.microtask(() => ref.read(finishedProgramsLogic.notifier).load());
  }
  ...
  void _listenToLogics(BuildContext context) {
    ref.listen<ProgramPatchState>(programPatchLogic, (previous, next) {
      ...
      if (next.phase == ProgramPatchPhase.archived) {
        ref.read(finishedProgramsLogic.notifier).removed(next.program);
    ...
    ref.listen(refreshLogic, (previous, next) {
      final key = ref.read(finishedProgramsLogic.notifier).hasRefreshKey(next);
      if (key == null) return;
      ref.read(refreshLogic.notifier).clear(key);
      ref.read(finishedProgramsLogic.notifier).load();
    });
  }
...
class _GridWidget extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final succeed = ref.watch(finishedProgramsLogic) as ProgramsSucceed;
    final programs = succeed.programs;
    return PullToRefresh(
      onRefresh: () => ref.read(finishedProgramsLogic.notifier).refresh(),
